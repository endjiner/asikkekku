...
if (!function_exists('role_position')) {
	...
	{
		$ci = &get_instance();
		return (string) $ci->session->userdata('UserPosition');
	}
}
...
if (!function_exists('role_is_admin')) {
	function role_is_admin()
	{
		...
		return $ci->session->userdata('UserGroupID') == 1 || role_position() === 'SuperAdmin';
	}
}
...
if (!function_exists('role_features')) {
	...
	{
		if (role_is_admin()) {
			return array('dashboard','manajemen_app','kegiatan_list','kegiatan_create','approval_inbox','report');
		}

		switch (role_position()) {
			case 'PJ-Kegiatan':
				// Pemohon/penyusun: hanya kelola datanya sendiri + kirim approval
				return array('dashboard','kegiatan_list','kegiatan_create');
			...
			case 'PPSPM':
				...
				return array('dashboard','kegiatan_list','approval_inbox','report');
