...
<div class="login-box">
  <div class="login-card">
    ...
    <div class="card-body">
      <?php if (!empty($this->session->userdata('error_info'))): ?>
        <div class="login-alert"><?php echo $this->session->userdata('error_info'); $this->session->set_userdata('error_info', ''); ?></div>
      <?php endif ?>
      <form method="post" action="<?=base_url('MainPage/VerifyLogin')?>">
        <?php $__ci =& get_instance(); ?>
        <input type="hidden" name="<?= $__ci->security->get_csrf_token_name() ?>" value="<?= $__ci->security->get_csrf_hash() ?>">
