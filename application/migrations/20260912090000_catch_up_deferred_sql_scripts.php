 ...
 * `php index.php migrate`) dan tidak pernah dipindah jadi migrasi CI3
 ...
 */
class Migration_Catch_up_deferred_sql_scripts extends CI_Migration
{
	public function up()
	{
		// 1. tb_wa_log (log pengiriman WhatsApp) + token di tb_vrbl -----------
		if (!$this->db->table_exists('tb_wa_log')) {
			$this->db->query("
		...
		if ($this->db->get_where('tb_vrbl', array('VrblName' => 'wa_token'))->num_rows() === 0) {
			$this->db->insert('tb_vrbl', array('VrblName' => 'wa_token', 'VrblValue' => 'ISI_TOKEN_WABLAS_DI_SINI'));
		}

		// 2. Fitur "Batalkan / Revisi": jenis pengembalian di riwayat approval -
		if (!$this->db->field_exists('FlowRejectType', 'tb_approval_history')) {
		...
		foreach ($menuOrder as $kode => $order) {
			$this->db->where('MenuKode', $kode)->update('tb_menu', array('MenuOrder' => $order));
		}
		$menuNama = array(
			'1000' => 'Dasbor',
		...
		foreach ($menuNama as $kode => $nama) {
			$this->db->where('MenuKode', $kode)->update('tb_menu', array('MenuName' => $nama));
		}
		...
		if (!$this->db->table_exists('tb_dokumen')) {
			$this->db->query("
		...
		if (!$this->db->field_exists('PayloadHash', 'tb_dokumen')) {
		...
		if ($this->db->get_where('tb_menu', array('MenuKode' => '3400'))->num_rows() === 0) {
		...
		$menuDokumen = $this->db->get_where('tb_menu', array('MenuKode' => '3400'))->row_array();
		$menuDaftar  = $this->db->get_where('tb_menu', array('MenuKode' => '3100'))->row_array();
		if ($menuDokumen && $menuDaftar) {
			$grupPunyaAkses = $this->db->select('UserGroupID')
				->where('MenuID', $menuDaftar['MenuID'])->where('Status_R', 1)
				->get('tb_menu_access')->result_array();
			foreach ($grupPunyaAkses as $g) {
				$sudahAda = $this->db->get_where('tb_menu_access', array(
					'MenuID' => $menuDokumen['MenuID'], 'UserGroupID' => $g['UserGroupID'],
				))->num_rows() > 0;
				if (!$sudahAda) {
					$this->db->insert('tb_menu_access', array(
						'MenuID' => $menuDokumen['MenuID'], 'UserGroupID' => $g['UserGroupID'],
		...
		if (!$this->db->table_exists('tb_dok_upload_ttd')) {
			$this->db->query("
		...
		if (!$this->db->table_exists('tb_ew_notifikasi')) {
			$this->db->query("
		...
		foreach ($slaDefault as $nama => $nilai) {
			if ($this->db->get_where('tb_vrbl', array('VrblName' => $nama))->num_rows() === 0) {
				$this->db->insert('tb_vrbl', array('VrblName' => $nama, 'VrblValue' => $nilai));
			}
		}
	}
	...
	{
		$this->db->delete('tb_vrbl', array('VrblName' => array('sla_total_hk', 'sla_warn_total_hk', 'sla_warn_stage_hk', 'ew_cron_key', 'wa_token')));
		...
		$menuDokumen = $this->db->get_where('tb_menu', array('MenuKode' => '3400'))->row_array();
		if ($menuDokumen) {
			$this->db->delete('tb_menu_access', array('MenuID' => $menuDokumen['MenuID']));
			$this->db->delete('tb_menu', array('MenuKode' => '3400'));
		}
