 ...
 */
class Migration_Dokumen_intake_pelaksana extends CI_Migration
{
	public function up()
	{
		...
		foreach ($userCols as $col => $def) {
			if (!$this->db->field_exists($col, 'tb_users')) {
		...
		$ada = $this->db->get_where('tb_vrbl', array('VrblName' => 'dok_output_list'))->num_rows() > 0;
		if (!$ada) {
			...
			$this->db->insert('tb_vrbl', array('VrblName' => 'dok_output_list', 'VrblValue' => $list));
		}

		// 5. Backfill: pecah KegiatanNamaPelaksana lama -> baris pelaksana
		if ($this->db->field_exists('KegiatanNamaPelaksana', 'tb_kegiatan')) {
			$rows = $this->db->query("
				...
				foreach ($parts as $nama) {
					...
					$u = $this->db->query(
						"SELECT UserID, UserName, UserGol, UserJabatan, UserRekening, UserBank, UserNPWP
						 FROM tb_users WHERE UserFullName = ? LIMIT 1", array($nama))->row_array();
	...
	public function down()
	{
		...
		foreach (array('KegiatanKodeOutput', 'KegiatanAsalTujuan', 'KegiatanJmlHari') as $c) {
			if ($this->db->field_exists($c, 'tb_kegiatan')) $this->db->query("ALTER TABLE `tb_kegiatan` DROP COLUMN `$c`");
		}
		foreach (array('UserGol', 'UserJabatan', 'UserRekening', 'UserBank', 'UserNPWP') as $c) {
			if ($this->db->field_exists($c, 'tb_users')) $this->db->query("ALTER TABLE `tb_users` DROP COLUMN `$c`");
		}
		$this->db->delete('tb_vrbl', array('VrblName' => 'dok_output_list'));
	}
}
