 ...
 */
class Migration_Jenis_pengajuan_configurable_flow extends Migration
{
    public function up()
    {
        ...
        $adaPerjadin = $this->db->table('tb_jenis_pengajuan')->getWhere(array('JenisKode' => 'PERJADIN'))->getNumRows() > 0;
        if (!$adaPerjadin) {
            $this->db->table('tb_jenis_pengajuan')->insert(array(
                'JenisID'     => 1,
                'JenisKode'   => 'PERJADIN',
        ...
        if (!$this->db->fieldExists('JenisID', 'tb_approval_flow')) {
            $this->db->query("ALTER TABLE `tb_approval_flow`
                ADD `JenisID` int(11) NOT NULL DEFAULT 1 AFTER `FlowPosition`");
        }
        if (!$this->db->fieldExists('FlowIsRevisiTarget', 'tb_approval_flow')) {
            $this->db->query("ALTER TABLE `tb_approval_flow`
        ...
        if (!$this->db->fieldExists('KegiatanJenisID', 'tb_kegiatan')) {
