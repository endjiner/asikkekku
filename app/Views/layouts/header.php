<head>
  ...
  <meta name="csrf-name" content="<?= $__security->getTokenName() ?>">
  <meta name="csrf-hash" content="<?= (string) $__security->getHash() ?>">
  <script>window.CSRF = { name: <?= json_encode($__security->getTokenName()) ?>, hash: <?= json_encode((string) $__security->getHash()) ?> };</script>
  ...
  <link rel="stylesheet" href="<?= base_url() ?>assets/css/custom-jtp.css?v=<?= file_exists(FCPATH . 'assets/css/custom-jtp.css') ? filemtime(FCPATH . 'assets/css/custom-jtp.css') : '20260923-ui8' ?>">
