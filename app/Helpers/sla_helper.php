 ...
 *
 * Porting dari application/helpers/sla_helper.php (CI3). Satu-satunya
 * perubahan: get_instance()->db -> db_connect() (helper global CI4) +
 * rename method query builder ke gaya CI4.
 */
...
if (! function_exists('sla_config')) {
    ...
    function sla_config()
    {
        ...
        try {
            $db   = db_connect();
            $rows = $db->table('tb_vrbl')->whereIn('VrblName', array_keys($cfg))->get()->getResultArray();
            foreach ($rows as $r) {
                $v = (int) $r['VrblValue'];
                if ($v > 0) {
                    $cfg[$r['VrblName']] = $v;
...
if (! function_exists('sla_badge')) {
    ...
    {
        if (empty($eval) || $eval['level'] === 'draf') {
            return '';
        }

        $icon = 'clock';
        if ($eval['level'] === 'terlambat' || $eval['level'] === 'selesai_telat') {
            $icon = 'warning';
        }
        if ($eval['level'] === 'selesai_ontime') {
            $icon = 'check-double';
        }
...
if (! function_exists('sla_teks_publik')) {
    ...
    {
        if (empty($eval) || $eval['level'] === 'draf') {
            return 'Berkas sedang disusun oleh petugas.';
        }
        switch ($eval['level']) {
            case 'selesai_ontime':
            case 'selesai_telat':
