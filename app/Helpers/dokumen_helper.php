 ...
 * Dipakai template pratinjau (app/Views/dokumen/tpl/*).
 *
 * Porting 1:1 dari application/helpers/dokumen_helper.php (CI3) -- murni
 * PHP, tidak menyentuh apa pun yang spesifik ke framework, jadi tidak ada
 * yang perlu diubah.
 */
...
if (! function_exists('terbilang')) {
    function terbilang($n)
    {
        ...
        $bilang = function ($x) use (&$bilang, $angka) {
            ...
            if ($x < 100) {
                return trim($bilang(intdiv($x, 10)) . ' puluh ' . $bilang($x % 10));
            }
            if ($x < 200) {
                return trim('seratus ' . $bilang($x - 100));
            }
            if ($x < 1000) {
                return trim($bilang(intdiv($x, 100)) . ' ratus ' . $bilang($x % 100));
            }
            if ($x < 2000) {
                return trim('seribu ' . $bilang($x - 1000));
            }
            if ($x < 1000000) {
                return trim($bilang(intdiv($x, 1000)) . ' ribu ' . $bilang($x % 1000));
            }
            if ($x < 1000000000) {
                return trim($bilang(intdiv($x, 1000000)) . ' juta ' . $bilang($x % 1000000));
            }
            if ($x < 1000000000000) {
                return trim($bilang(intdiv($x, 1000000000)) . ' miliar ' . $bilang($x % 1000000000));
            }
...
if (! function_exists('tgl_ind')) {
    function tgl_ind($date, $with_day = false)
    {
        ...
        $bln = [1 => 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli',
